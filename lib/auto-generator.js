    ...
    addAssociations(typeScript, model) {
        ...
        rels.forEach((rel) => {
            if (rel.isM2M && rel.parentModel === model) {
                ...
                strBelongsToMany += `    this.belongsToMany(models.${types_1.recase(this.options.caseProp, rel.childModel)}, { as: '${asprop}', through: '${types_1.recase(this.options.caseProp, rel.joinModel)}', foreignKey: '${rel.parentId}', otherKey: '${rel.childId}' });\n`;
            }
            else {
