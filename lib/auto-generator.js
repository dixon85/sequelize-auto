...
class AutoGenerator {
            ...
            if (this.options.lang === 'ts') {
                ...
                if (primaryKeys.length) {
                    str += `export type #TABLE#Pk = ${primaryKeys
                        .map((k) => `"${types_1.recase(this.options.caseProp, k)}"`)
                        .join(' | ')};\n`;
    ...
    addAssociations(typeScript, model) {
        let strBelongs = '';
        ...
        rels.forEach((rel) => {
            if (rel.isM2M && rel.parentModel === model) {
                ...
                strBelongsToMany += `    this.belongsToMany(models.${types_1.recase(this.options.caseProp, rel.childModel)}, {\n      through: '${types_1.recase(
                //)}, {\n      as: '${asprop}',\n      through: '${recase(
                this.options.caseProp, rel.joinModel)}',\n      foreignKey: '${rel.parentId}',\n      otherKey: '${rel.childId}',\n    });\n`;
            }
            else {
                // check if M2M exists
                let existsM2M = false;
                rels.find((relation) => {
                    if (relation.isM2M === true &&
                ...
                if (rel.childModel === model && existsM2M === false) {
                    const bAlias = this.options.noAlias && rel.parentModel.toLowerCase() === rel.parentProp.toLowerCase()
                        ? ''
                        : `as: '${rel.parentProp}', `;
                    strBelongs += `    this.belongsTo(models.${types_1.recase(this.options.caseProp, rel.parentModel)}, { ${bAlias}foreignKey: '${rel.parentId}' });\n`;
                }
                existsM2M = false;
                rels.find((relation) => {
                ...
                if (rel.parentModel === model && existsM2M === false) {
                    ...
                    const hAlias = this.options.noAlias && sequelize_1.Utils.pluralize(rel.childModel.toLowerCase()) === rel.childProp.toLowerCase()
                        ? ''
                        : `as: '${rel.childProp}', `;
                    strBelongs += `    this.${hasRel}(models.${types_1.recase(this.options.caseProp, rel.childModel)}, { ${hAlias}foreignKey: '${rel.parentId}' });\n`;
                }
            }
        });
    ...
    /** Get the sequelize type from the Field */
    getFieldValidation(fieldObj, attr) {
        const attrValue = fieldObj[attr];
        if (!attrValue.toLowerCase) {
            console.log('attrValue', attr, attrValue);
            return attrValue;
        ...
        if (type === 'boolean' || type === 'bit(1)' || type === 'bit' || type === 'tinyint(1)' || type === 'tinyint') {
            ...
            val += `${this.space[3]}${this.space[3]}${this.space[1]}msg: '${fieldObj.defaultValue}  must be no more than ${length} characters in length.',\n`;
            val += `${this.space[3]}${this.space[3]}},\n`;
        }
        else if (type.match(/^n?char/)) {
            val = '';
            ...
            if (!rel.isM2M) {
                if (rel.childTable === table) {
                    ...
                    if (rel.isOne) {
                        ...
                        const sing = lodash_1.default.upperFirst(types_1.singularize(rel.childProp));
                        const lur = types_1.pluralize(rel.childProp);
                ...
                if (rel.parentTable === table) {
                    ...
                    const otherModelSingular = lodash_1.default.upperFirst(types_1.singularize(isParent ? rel.childProp : rel.parentProp));
                    const lotherModelPlural = types_1.pluralize(isParent ? rel.childProp : rel.parentProp);
    ...
    isTimestampField(field) {
        ...
        return ((!additional.createdAt && field.toLowerCase() === 'createdat') ||
