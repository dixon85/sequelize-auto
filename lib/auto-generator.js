        ...
        fieldAttrs.forEach((attr) => {
            ...
            if (attr === 'foreignKey') {
                if (foreignKey && foreignKey.isForeignKey) {
                    ...
                    str += space[6] + "model: '" + fieldObj[attr].foreignSources.target_table + "',\n";
                    str += space[6] + "key: '" + fieldObj[attr].foreignSources.target_column + "'\n";
                    str += space[5] + '}';
                }
                else {
                    return true;
    ...
    addIndexes(table) {
        ...
        if (indexes && indexes.length) {
            ...
            indexes.forEach((idx) => {
                str += space[5] + '{\n';
                if (idx.name) {
                    str += space[6] + `name: "${idx.name}",\n`;
                }
                if (idx.unique) {
                    str += space[6] + 'unique: true,\n';
                }
                if (idx.type) {
                    if (['UNIQUE', 'FULLTEXT', 'SPATIAL'].includes(idx.type)) {
                        str += space[6] + `type: "${idx.type}",\n`;
                    }
                    else {
                        str += space[6] + `using: "${idx.type}",\n`;
                    }
                }
                str += space[6] + `fields: [\n`;
